...
class FisIcerikViewModel extends ChangeNotifier {
  ...
  FisIcerikViewModel(
    ...
    DatabaseService.instance.firmDb
        .mySelectQuery("select ID,Barcode,Name,Aciklama,Miktar from V_AllItems")
        .then((value) {
      productList = value;
      retriveAllRows();
    });
  ...
  Future addTrnLine() async {
    ...
    await addProduct();
    if (fisSatirlari.any((element) => element.productId == urun!.id)) {
      await updateLine(
  ...
  orderLinesByDate() {
    ...
    fisSatirlari.sort((a, b) => time
        .fromDateTimeString(b.date)
        .compareTo(time.fromDateTimeString(a.date)));
    notifyListeners();
  }
}
